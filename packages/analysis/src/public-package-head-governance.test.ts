...
describe("public package S/A/C HEAD governance", () => {
  it("classifies source-only HEADs as S only before the v7 receipt exists", () => {
  ...
  it("requires exact, single-purpose runtime input and candidate commits", () => {
    ...
    const oldCandidate = "output/compute-service-candidate-5d2714e";
  ...
  it("strictly binds the runtime input to receipt S and the v7 release", () => {
    const input = {
      ...
      approvedLongitudinalBuild: {
        ...
        sdkVersion: "0.2.0-implemented-unverified.7",
        buildId: sourceHead,
      },
      migrations: [
    ...
    expect(PUBLIC_PACKAGE_RUNTIME_INPUT_PATH).toContain("implemented-unverified.7");
