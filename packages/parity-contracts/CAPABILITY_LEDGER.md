...
| SERVICE-002 | PostgreSQL/S3 persistent state and objects | `TS_SUCCESSOR_VERIFIED` | `IMPLEMENTED_UNVERIFIED` | exact `0001-persistent-compute` expand migration is active on the Owner's Singapore Neon `main/neondb` with one hash-bound registry row, 17 tables, append-only rejection, transaction rollback and child-branch reset/deletion probes; independent approval, BuildApproval-bound preview isolation, Fly replicas, object storage, restart/capacity/TTL/object-deletion, fencing and soak receipts remain |
