...
| SDK-001 | One public `@3dena/analysis` facade | `TS_SUCCESSOR_VERIFIED` | `IMPLEMENTED_UNVERIFIED` | local V2 prerelease tarball passes Node/strict-TS/Vite/Next-webpack consumers with declarations/maps/schemas/notices/provenance; it requires exactly one reviewed `jena-js@0.7.0-ona.0` peer and does not bundle a second engine; registry publication and independent API/release review remain |
...
| ORACLE-001 | Historical R/rENA compatibility artifacts | `FROZEN_COMPATIBILITY_ARTIFACT` | custody candidate | frozen environment, hashes and production exclusion; not a Longitudinal V2 numerical oracle or release gate |
...
| REL-001 | Clean CI, exact artifacts, security and licenses | `TS_SUCCESSOR_VERIFIED` | `IMPLEMENTED_UNVERIFIED` | reviewed `jena-js@0.7.0-ona.0` source/tarball custody, exact peer policy, local release-security and tarball-consumer gates are implemented; exact-head remote CI, independent review, container/preview/soak/rollback and complete release receipts remain |
